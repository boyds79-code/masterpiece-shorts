 ...
 * 단정적 서술("~였다")이 아니라 추정 어조("~였을 것이다", "~했을 가능성이 높다")를 강제합니다.
 * 영상 자체에 "AI 상상 재현"이라는 문구를 코드 레벨(제목/설명/인트로 카드)에서 강제로 붙이는
 * 건 이 함수를 호출하는 쪽(generate-process-video.mjs)의 책임입니다 — Claude의 서술에만
 * 의존하면 매번 빠짐없이 지켜진다는 보장이 없기 때문입니다.
 *
 ...
 * sketch/underpainting/refine 단계와 그 steps가 이 근거를 따르도록 강제합니다.
 *
 * 세그먼트는 정해진 순서를 따르고(총 6개), 인트로/아웃트로 카드(약 5.5초)를 더하면 전체
 * 영상이 유튜브 쇼츠에 맞는 1~2분대가 되도록 나레이션 분량을 조정합니다:
 * 1. identify (실제 사진, 전체 화면) — 언제: 완성작이 무엇인지, 언제 그려졌는지 소개.
 * 2. reference (실제 사진, 전체 또는 특정 부분 확대) — 왜: 왜 이렇게 그려졌는지(누구를 위해,
 *    어떤 동기/맥락/목적으로) + 작가가 무엇을 관찰/참고했을지 상상.
 * 3~5. sketch, underpainting, refine (각각 AI가 새로 생성한 이미지) — 어떻게: techniqueBasis에
 *    따른 초기 스케치, 밑칠/명암 구축, 마무리 직전 단계를 상상해서 묘사. 이 세 세그먼트는
 *    "steps"(PROCESS_STEPS_PER_STAGE개의 순차적인 진행 컷 프롬프트)를 받아, 각 컷이 직전 컷
 *    이미지를 이어받아 체이닝 생성되고 영상에서는 짧은 와이퍼 전환(화면은 고정된 채 다음
 *    컷이 왼쪽에서 오른쪽으로 쓸려 나오며 이전 컷을 덮음)으로 이어붙여 타임랩스처럼 보입니다. "어떻게" 파트가 너무
 *    길게 늘어진다는 피드백에 따라, 이 세 세그먼트는 각각 나레이션을 짧게(18-24단어) 잡아서
 *    셋을 합쳐 30초 안쪽이 되도록 강제합니다 — 세그먼트 영상 길이는 나레이션 오디오 길이를
 *    그대로 따라가므로(generate-process-video.mjs가 TTS 오디오 길이를 durationSec으로 씀),
 *    나레이션을 짧게 잡는 것이 곧 화면 길이를 줄이는 방법입니다.
 * 6. finish (실제 사진, 전체 화면): 완성작으로 돌아와 언제·왜·어떻게를 하나로 엮고, 완성된
 *    작품이 전체적으로 주는 인상/의미에 대한 짧은 한마디를 더해 마무리합니다(구체적인 숨은
 *    상징/디테일 해석은 여전히 별도의 "숨은 의미" 영상의 몫이므로, 여기서는 분위기/색감/구도가
 *    전체적으로 자아내는 느낌 수준의 일반적인 감상만 담습니다).
 */
...
STEP 2 — structure the script as exactly 6 segments, in exactly this order and stage type, consistent with the techniqueBasis you just stated. Together these 6 segments must leave the viewer understanding WHEN the painting was made, WHY it was made this way, and HOW it was made — and, briefly at the very end, what the finished piece as a whole seems to evoke or express. A separate, paired video does a deep dive into this painting's SPECIFIC hidden symbolic details (concealed objects, coded references, decoded Easter eggs, etc.) — so do NOT reveal or hint at any of those specific hidden details here. The "finish" stage may still include one broad, general observation about the completed work's mood, symbolism, or emotional effect as a whole (e.g., what the composition, color palette, or subject as a whole seems to express) — keep that general and brief, never a specific decoded secret that would step on the paired video's territory:
1. stage "identify" (usesGeneratedImage: false, bbox = the whole painting): Introduce the finished painting — title, artist, rough year (WHEN). End with a hook promising to reconstruct how it might have come together, stroke by stroke.
2. stage "reference" (usesGeneratedImage: false, bbox = whole painting or a specific real detail): Cover WHY this painting exists and looks the way it does — imagine the artist's motivation, purpose, or context (who it may have been made for or commissioned by, what personal/historical circumstances shaped it, what belief/emotion/event it was responding to) AND what the artist likely observed, studied, or referenced to build the composition (a live model, a study of natural light, a religious/mythological source text, preliminary drawings, a specific setting) — grounded in what the finished image actually shows and in the techniqueBasis.
3. stage "sketch" (usesGeneratedImage: true): Describe the imagined initial stage consistent with techniqueBasis — either a loosely blocked-in drawn underdrawing (for artists/movements known to work that way) or a rough gestural color block-in with no drawn stage (for artists/movements known to paint alla prima, skipping a formal underdrawing). Instead of one single image, write "steps": an array of exactly 3 short image-generation prompts, each describing ONE INCREMENTAL bit of progress within this stage, from barest beginning to this stage's natural end point (e.g., step 1 = the very first loose marks establishing the main shapes only; step 2 = more of the composition roughed in, still loose and incomplete; step 3 = this early stage essentially complete, ready to move to the next stage). Each step's image will be generated by continuing from the ACTUAL image produced by the previous step (not from scratch) — so word each step as an incremental instruction ("add...", "rough in...", "extend the ... further") rather than restating the whole scene, and name the actual technique (e.g., "loose charcoal underdrawing" vs. "rough alla-prima color block-in with visible loose brushstrokes, no pencil lines"). Keep this stage's "narration" short — roughly 18-24 words (one short sentence plus a fragment) — since sketch/underpainting/refine together must total well under 30 seconds of spoken narration; do not pad it with filler.
4. stage "underpainting" (usesGeneratedImage: true): Describe the imagined tonal/color block-in stage consistent with techniqueBasis — e.g., a monochrome grisaille underpainting for a glazing-based technique, or a direct rough color mass block-in for alla prima work. Write "steps": 3 incremental prompts (same rules as above) taking the composition from where "sketch" left off toward a more built-up tonal/color block-in, naming the specific technique. Keep "narration" short here too — roughly 18-24 words — self-contained (no references to the sketch segment by name like "as we just saw"), for the same total-runtime reason as above.
5. stage "refine" (usesGeneratedImage: true): Describe the imagined final push consistent with techniqueBasis — layering transparent glazes and sharpening edges for a glazing technique, or adding thicker impasto highlights and final direct strokes for an alla-prima/impasto technique. Write "steps": 3 incremental prompts taking the composition from the underpainting stage to nearly-finished (the very last step should look close to, but not quite, the real finished painting), naming the specific technique. Keep "narration" short here too — roughly 18-24 words — so that sketch + underpainting + refine narration combined lands well under 30 seconds of spoken audio (this stage's steps still get their normal on-screen time from the fixed per-image slide pacing; only the SPOKEN narration needs to be this brief).
6. stage "finish" (usesGeneratedImage: false, bbox = the whole painting): Pull back to the real finished painting and tie WHEN/WHY/HOW together into one closing thought. Then add ONE brief, general observation about what the completed piece as a whole seems to evoke or express — its overall mood, the emotional effect of its composition/color/light, or what it broadly represents (stay general and impressionistic; do NOT decode a specific hidden symbol, object, or Easter egg — that specific-detail territory belongs to the separate paired "hidden meaning" video). Close with a light line reinforcing that the HOW portion was an imagined reconstruction grounded in known technique, not a documented record (e.g., "That's our best guess at how it came together, built on how [movement] painters actually worked."). Narration target for this stage: roughly 55-75 words (longer than the other segments since it now carries this extra closing thought).

For every segment:
- "narration": natural spoken chunks, no references to "earlier" segments. For identify/reference/finish, TWO to FOUR short sentences; for sketch/underpainting/refine, keep it to ONE short sentence plus a fragment (per the tight word caps given for those stages above — do not use four sentences there). At least one segment among sketch/underpainting/refine should naturally mention the real technique term from techniqueBasis (e.g., "alla prima", "glazing", "grisaille underpainting") so the reconstruction reads as informed, not generic.
...
Per-segment narration targets: identify roughly 30-40 words; reference roughly 45-60 words; sketch/underpainting/refine roughly 18-24 words EACH (this is a hard cap — the three combined must stay well under 30 seconds of spoken narration, since each segment's on-screen duration is driven directly by its narration audio length); finish roughly 55-75 words. Total across all 6 segments: roughly 185-250 words (this becomes roughly 75-105 seconds of spoken narration). Combined with the fixed ~5.5-second intro/outro title cards, the finished video should land in the 1-2 minute range that works well for YouTube Shorts — do not go noticeably short of this range, and keep it from stretching much past 2 minutes total. Do NOT let the sketch/underpainting/refine segments run long to compensate for a short identify/reference/finish — the HOW portion staying brief is the point.
