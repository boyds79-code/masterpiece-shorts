...
**항상 `npm run generate:review`로 확대 위치(bbox)를 브라우저에서 직접 검토한 뒤에만
만들어집니다** — 예전엔 GitHub Actions로 하루 2회 자동 실행됐지만, 사람 검토 없이 줌인이
틀린 채로 올라간 적이 있어서 자동 스케줄에서 뺐습니다(아래 "확대 위치(bbox) 미리
검토하기" 섹션 참고). 자동으로 계속 도는 건 아래 "제작 과정 상상 재현" 파이프라인뿐입니다
(하루 2회 — 자세한 스케줄은 "자동 실행 스케줄" 섹션 참고).
...
1. `npm run generate:review`를 실행하면 그림 선정과 대본(bbox 포함) 생성까지 끝난 뒤
   브라우저 검토 화면이 뜹니다 — 확대 위치를 확인/수정하고 "실행하기"를 눌러야 비로소
   나레이션/영상 조립/업로드가 진행됩니다 (아래 "확대 위치(bbox) 미리 검토하기" 섹션
   참고). `.github/workflows/daily-video.yml`은 더 이상 예약(schedule) 실행되지 않고
   `workflow_dispatch`로만 남아있습니다 — 검토 없이 예전 방식대로 한 번 돌려보고 싶을
   때만 Actions 탭에서 수동 실행하세요.
...
5. ffmpeg으로 각 구간마다 해당 영역을 확대/팬(Ken Burns 효과)한 뒤, 인트로(제목 카드) +
   ...
   굽지 않습니다 — 대신 나레이션과 시간이 맞는 SRT 자막 파일을 만듭니다. IDENTIFY/CONTEXT/
   CLOSE처럼 bbox가 전체 그림인 구간에서, 그림 자체가 가로로 넓은(landscape) 구도라면
   좌우를 잘라내는 대신(예: 마네 "뱃놀이"처럼 넓은 작품에서 전체 구도가 한 번도 안 보이는
   문제 방지) 그림 전체를 축소해 중앙에 배치하고 남는 위/아래 여백은 같은 그림을 블러
   처리한 배경으로 채웁니다(`scripts/lib/video-builder.mjs`의 `buildSegmentClip()` 참고).
...
### 7. 첫 실행 테스트
로컬에서 `npm run generate:review`를 실행해 브라우저 검토 화면까지 잘 뜨는지, "실행하기"를
눌렀을 때 나레이션/영상 조립/업로드가 끝까지 잘 되는지 확인하세요. (`daily-video.yml`은
`workflow_dispatch`만 남아있어서 저장소 **Actions 탭 → Generate masterpiece short → Run
workflow**로도 검토 없이 한 번 실행해볼 수 있지만, 평소엔 쓰지 않는 걸 권장합니다 — 아래
"확대 위치(bbox) 미리 검토하기" 섹션 참고.) 실행이 끝나면 로그 마지막 줄에 뜨는 YouTube
Studio 링크로 들어가서 결과를 확인하세요.

## 자동 실행 스케줄 (하루 2개)

자동으로 매일 도는 건 "제작 과정 상상 재현" 파이프라인(`process-video.yml`)뿐입니다 — 하루
2회, UTC 기준(https://crontab.guru) 01:05/13:05:
...
"숨은 의미" 파이프라인(`daily-video.yml`)은 자동 스케줄에서 제외했습니다 — 각 구간의 확대
위치(bbox)가 조금만 틀려도 바로 티가 나는 형식인데, 사람 검토 없이 자동으로 올라갔다가
줌인이 엉뚱한 채로 올라간 적이 있어서입니다. 이제 "숨은 의미" 영상은 항상
`npm run generate:review`로 브라우저에서 bbox를 직접 확인한 뒤에만 만들어집니다(아래
"확대 위치(bbox) 미리 검토하기" 섹션 참고).

**쿼터 한도**
YouTube Data API의 기본 일일 쿼터는 **10,000 units**이고, 영상 하나를 올릴 때
`videos.insert`(1,600) + `captions.insert`(200) + `thumbnails.set`(50, 제작 과정
파이프라인만 해당) = 최대 약 1,850 units가 듭니다. 자동 실행 2회 × 1,850 = 3,700
units에, 수동으로 `generate:review`를 몇 번 더 돌려도 기본 쿼터 안에 넉넉히 들어옵니다.
쿼터는 태평양 표준시 자정에 초기화됩니다. 더 자주(또는 더 많이) 올리고 싶다면 [Google
Cloud Console에서 이 프로젝트의 YouTube Data API 쿼터 증설을 먼저
신청](https://support.google.com/youtube/contact/yt_api_form)하세요(승인에 며칠 걸릴 수
있습니다) — 승인되면 `process-video.yml`의 `cron` 줄에 시간을 추가하면 됩니다.
...
**그림 소진 속도 참고**
"숨은 의미"(`generate:review`, 수동)와 "제작 과정"(`process-video.yml`, 자동 하루 2회)
파이프라인이 `data/used-paintings.json`을 공유합니다. `pickUnusedPainting()`이 더 이상
고를 그림이 없으면 `null`을 반환하고 그 실행은 조용히 아무것도 안 올린 채 끝납니다 — 이
상태가 되면 `scripts/lib/met-api.mjs`의 `DEPARTMENT_IDS`에 다른 부서(예: 미국 회화, 근대
유럽 회화 등)를 추가해서 소재 풀을 늘려야 합니다.

## 확대 위치(bbox) 미리 검토하기 — "숨은 의미" 필수 절차

"숨은 의미" 영상은 각 구간마다 그림의 특정 부분을 확대해서 보여주는데, Claude가 대본과
동시에 그 확대 좌표(bbox)도 정합니다. 자체적으로 gridPosition 대조 + AI 재검증(최대
3회)까지 하지만, 그래도 가끔 엉뚱한 부분을 확대하는 경우가 있습니다 — 실제로 사람 검토
없이 자동으로 올린 영상 중에 강조하려던 부분이 화면 구석에만 살짝 걸리거나, 언급한
디테일(반지 등)이 아예 화면에 안 잡힌 채로 올라간 적이 있었습니다. 그래서 `npm run
generate` 대신 아래처럼 실행해서, 영상을 만들기 전에 확대 위치를 브라우저에서 직접 보고
고치는 절차를 항상 거칩니다 — 터미널을 다시 건드릴 필요 없이 그 화면에서 저장하고 바로
실행까지 됩니다.
...
3. 6단계 대본을 씁니다: (1) identify — 언제: 완성작 소개 → (2) reference — 왜: 누구를
   ...
   하나로 엮고, 완성작이 전체적으로 주는 인상/의미에 대한 짧은 한마디를 더해 마무리합니다.
   구체적인 숨은 상징/디테일 해석은 여전히 다루지 않습니다 — 그건 별도의 "숨은 의미"
   파이프라인의 몫이고, 여기서는 분위기/색감/구도가 자아내는 느낌 수준의 일반적인 감상만
   담습니다. "어떻게"(3~5) 세그먼트는 대본 생성 시점부터 각각 나레이션을 18-24단어로 짧게
   제한해서 셋을 합쳐 30초 안쪽이 되도록 강제합니다 — "어떻게" 파트가 너무 길게 늘어진다는
   피드백을 반영한 것입니다. 유튜브 쇼츠에 맞게 인트로/아웃트로 카드를 포함해 전체 영상이
   1~2분 사이가 되도록 나레이션 분량을 맞춥니다(세그먼트별 정확한 단어 수 목표는
   `scripts/lib/process-script.mjs`의 시스템 프롬프트 참고).
4. "초기 단계"/"밑칠"/"마무리 직전" 3단계는 실제로 존재하는 이미지가 아니므로, Gemini 이미지
   ...
   영상에서는 이 진행 컷들을 단계에 따라 서로 다른 전환으로 이어붙입니다
   (`scripts/lib/video-builder.mjs`의 `buildTimelapseSegmentClip()`이 `style` 인자로
   구분합니다):
   - **sketch("스케치"): `'progressive'` 스타일** — 슬롯 전체 길이 동안 계속 조금씩
     번져 나가며 캔버스를 덮어가는 점진적 확장 전환입니다("붓결 순서 필드"라는 얼룩덜룩한
     그레이스케일 마스크를 기준으로 낮은 값부터 먼저 새 이미지로 바뀜). 선이 조금씩
     늘어나며 그려지는 스케치 특유의 느낌에 어울립니다.
   - **underpainting/refine("색칠"): `'wipe'` 스타일** — 화면(구도)은 고정된 채, 다음
     컷이 화면 왼쪽 끝에서부터 오른쪽으로 쓸려 나오며 이전 컷을 덮어 나가는 와이퍼
     전환입니다(이미지 자체는 이동하지 않습니다). 대부분의 시간은 정지 화면이고 단계가
     바뀔 때만 아주 짧게(0.4초) 넘어가서, 색을 채워나가는 단계는 "이전 단계 → 다음
     단계"가 또렷하게 구분되어 보입니다.

   시도 순서는 이랬습니다: 처음엔 모든 단계에 `'progressive'`(붓결 순서 필드 마스크) 하나만
   썼지만 실제 붓터치가 아니라 화면 곳곳이 "번지는" 것처럼 보인다는 피드백을 받아 전체를
   화면이 옆으로 밀려나는 슬라이드로 바꿨다가, 화면(구도)이 고정된 채 다음 레이어만
   와이퍼처럼 훑고 지나가야 한다는 피드백을 받아 전체를 `'wipe'`로 바꿨습니다. 이후 다시
   피드백을 받아, 스케치처럼 점진적으로 확장되며 그려지는 단계는 원래의 `'progressive'`가
   더 어울리고 색을 채우는 단계는 또렷한 `'wipe'`가 낫다는 것으로 정리되어, 지금처럼
   스테이지별로 다른 스타일을 쓰게 됐습니다.
   `scripts/lib/process-script.mjs`의 `PROCESS_STEPS_PER_STAGE`(기본 3) 상수를 올리면
   진행 컷 수가 늘어나 더 여러 단계를 보여줄 수 있지만(정해진 짧은 나레이션 시간을 그만큼
   더 잘게 나눠 갖게 됩니다), 그만큼 Gemini 이미지 생성 호출 수와 생성 시간이 늘어납니다.
5. 각 단계 나레이션은 Gemini TTS로 음성 변환하고, ffmpeg으로 이어붙여 영상을 만듭니다.
   썸네일은 화면을 정확히 반으로 나눠 왼쪽엔 완성작 원본, 오른쪽엔 sketch 단계의 마지막
   진행 컷을 각각 꽉 채우고 그 사이에 얇은 구분선을 그어서, 썸네일만 보고도 "원본 →
   스케치"가 한눈에 대비되도록 만듭니다(`scripts/lib/video-builder.mjs`의
   `buildThumbnail()`이 `splitImagePath`를 받으면 이 2분할 방식을 쓰고, 안 받으면
   기존 letterbox 방식을 씁니다 — "숨은 의미"/긴 영상 파이프라인은 여전히 기존 방식).
