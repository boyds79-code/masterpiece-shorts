...
4. "초기 단계"/"밑칠"/"마무리 직전" 3단계는 실제로 존재하는 이미지가 아니므로, Gemini 이미지
   ...
   영상에서는 이 진행 컷들을 짧은 와이퍼 전환(화면·구도는 그대로 고정된 채, 다음 컷이 화면
   왼쪽 끝에서부터 오른쪽으로 쓸려 나오며 이전 컷을 덮어 나감 — 이미지 자체가 옆으로
   밀려나거나 이동하지는 않습니다)으로 이어붙입니다. 시도 순서는 이랬습니다: 처음엔
   "붓결 순서 필드"로 만든 마스크 기반 전환을 썼지만 실제 붓터치가 아니라 화면 곳곳이
   "번지는" 것처럼 보인다는 피드백을 받았고, 그다음 화면 전체가 옆으로 밀려나는 슬라이드
   전환을 시도했지만 화면(구도)이 움직이지 않고 제자리에 그대로 있는 채 다음 레이어만
   와이퍼처럼 훑고 지나가야 한다는 피드백을 받아 지금의 와이퍼 방식으로 다시 바꿨습니다.
   대부분의 시간은 각 진행 컷을 또렷한 정지 화면으로 보여주고 단계가 바뀔 때만 아주 짧게
   (0.4초) 와이퍼로 넘어가는 방식이라 "이전 단계 → 다음 단계"가 훨씬 분명하게 보입니다.
