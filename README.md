...
4. "초기 단계"/"밑칠"/"마무리 직전" 3단계는 실제로 존재하는 이미지가 아니므로, Gemini 이미지
   ...
   영상에서는 이 진행 컷들을 짧은 슬라이드 전환(다음 컷이 화면 왼쪽에서 들어오며 이전 컷을
   오른쪽으로 밀어냄)으로 이어붙입니다 — 예전에는 "붓결 순서 필드"로 만든 마스크 기반
   전환을 썼지만 실제 붓터치가 아니라 화면 곳곳이 "번지는" 것처럼 보인다는 피드백을 받아서,
   대부분의 시간은 각 진행 컷을 또렷한 정지 화면으로 보여주고 단계가 바뀔 때만 아주 짧게
   (0.4초) 슬라이드로 넘어가는 방식으로 바꿨습니다 — "이전 단계 → 다음 단계"가 훨씬 분명하게
   보입니다. `scripts/lib/process-script.mjs`의 `PROCESS_STEPS_PER_STAGE`(기본 3) 상수를
   올리면 진행 컷 수가 늘어나 더 여러 단계를 보여줄 수 있지만(정해진 짧은 나레이션 시간을
   그만큼 더 잘게 나눠 갖게 됩니다), 그만큼 Gemini 이미지 생성 호출 수와 생성 시간이
   늘어납니다.
